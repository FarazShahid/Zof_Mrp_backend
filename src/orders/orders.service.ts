...
export class OrdersService {
  ...
  async createOrder(
    ...
    try {
      ...
      const autoGeneratedOrderNumber = OrderNumber
        ? OrderNumber
        : `${client.Name?.[0] ?? ''}${OrderName?.[0] ?? ''}${client.Id}-${orderCount + 1}`;
      ...
      const newOrder = queryRunner.manager.create(Order, {
        ...
        OrderPriority,
        ExternalOrderId,
        OrderNumber: autoGeneratedOrderNumber,
        OrderName,
        OrderType,
        CreatedBy: createdBy,
        UpdatedBy: createdBy,
  ...
  async createChecklistForOrderItem(
    ...
    if (measurementIds.length > 0) {
      ...
      const parameterLabels: Record<string, string> =  {
        ...
        Bag_Length: 'Bag Length',
        Bag_Depth: 'Bag Depth',
        Bag_HandleGrip: 'Bag Handle Grip',
        Bag_ShoulderStrap_Full_Length: 'Bag Shoulder Strap Full Length',
        Bag_FrontPocketLength: 'Bag Front Pocket Length',
        Bag_FrontPocketHeight: 'Bag Front Pocket Height',
        Bag_SidePocketLength: 'Bag Side Pocket Length',
        Bag_SidePocketHeight: 'Bag Side Pocket Height',
        Cap_CrownCircumference: 'Cap Crown Circumference',
        Cap_BrimLength: 'Cap Brim Length',
        Cap_BrimWidth: 'Cap Brim Width',
        Cap_Height: 'Cap Height',
        Cap_Crown_Width: 'Cap Crown Width',
        cap_cuff_height: 'Cap Cuff Height',
      }
