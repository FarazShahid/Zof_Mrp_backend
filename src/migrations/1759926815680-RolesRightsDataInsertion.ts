...
export class RolesRightsDataInsertion1759926815680 implements MigrationInterface {
    name = 'RolesRightsDataInsertion1759926815680'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            INSERT INTO app_roles (id, name, CreatedOn, createdBy, UpdatedOn, updatedBy)
            VALUES (1, 'MRP Admin', NOW(), 'system', NOW(), 'system');
        `);

        await queryRunner.query(`
            INSERT INTO app_roles_rights (roleId, rightId, CreatedOn, createdBy, UpdatedOn, updatedBy)
            VALUES
            ${Array.from({ length: 68 }, (_, i) => `(1, ${i + 1}, NOW(), 'system', NOW(), 'system')`).join(',\n')};
        `);

        await queryRunner.query(`
            INSERT INTO users 
            (Email, Password, isActive, CreatedBy, UpdatedBy, firstName, lastName, roleId, assignedClients, CreatedOn, UpdatedOn)
            VALUES 
            ('superadmin@mrp.com', '$2b$10$32n2s8wSQwWSImFf.hCEZ.FMoHG73ZbcRAHvmSEGO8In5w2K1z5Dy', 1, 'System', 'System', 'MRP', 'Admin', 1, NULL, '2025-10-08 17:54:52', '2025-10-08 17:54:52');
        `);
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            DELETE FROM app_rights WHERE id BETWEEN 1 AND 68;
        `);

        await queryRunner.query(`
            DELETE FROM app_roles WHERE id = 1;
        `);

        await queryRunner.query(`
            DELETE FROM app_roles_rights 
            WHERE roleId = 1 
            AND rightId BETWEEN 1 AND 68;
        `);

        await queryRunner.query(`
            DELETE FROM users WHERE Email = 'superadmin@mrp.com';
        `);
