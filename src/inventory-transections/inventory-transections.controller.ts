...
export class InventoryTransectionController {
    ...
    async findOne(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: ValidatedUser) {
        try {
    ...
    async delete(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: ValidatedUser) {
        try {
